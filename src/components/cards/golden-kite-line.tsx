...
export default function GoldenKiteLine({ forecast, goldenWindow, className }: GoldenKiteLineProps) {
  ...
  useEffect(() => {
    ...
    // Filter forecast to visible date range
    ...
    // Calculate width based on data points - make it wider for scrolling
    const dataPointWidth = 40 // Width per data point
    const calculatedWidth = Math.max(containerWidth, limitedForecast.length * dataPointWidth)
    ...
    svg
      ...
      .call(
        d3
          .axisBottom(x)
          .ticks(5)
          .tickFormat((d) => format(d as Date, "MMM dd ha")), // Changed from "MMM dd" to "MMM dd ha"
      )
      .attr("font-size", "10px")
  ...
  }, [forecast, goldenWindow, visibleDateRange])
  ...
  return (
    ...
    >
      ...
      <CardContent className="space-y-4">
        {/* Golden window info - MOVED TO TOP */}
        ...
        <div className="relative">
          ...
          <div className="flex justify-between mt-2">
            <button
              onClick={() => {
                const newStart = new Date(visibleDateRange.start)
                const newEnd = new Date(visibleDateRange.end)
              ...
              disabled={visibleDateRange.start <= new Date()}
            >
              ← Previous Day
            </button>

            <div className="text-xs font-medium">{format(visibleDateRange.start, "MMM dd")}</div>
        ...
        {showDetails && (
          <div className="border-t border-amber-200/50 dark:border-amber-900/50 pt-4 mt-2 space-y-4">
            ...
            <div className="mt-6">
              <h4 className="text-lg font-semibold mb-4">Detailed Weather Forecast</h4>
              <FullForecastWeather
                forecast={forecast.map((item) => ({
                  ...
                  condition: item.rain > 0.5 ? "rainy" : item.temperature > 25 ? "sunny" : "cloudy",
                }))}
              />
            </div>
          </div>
